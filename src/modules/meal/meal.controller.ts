...
export class MealController {
    ...
    async createMeal(@Body() createMealPayload: CreateMealDto, image: Express.Multer.File): Promise<{ message: string, new_meal: CreateMealDto }> {
    ...
    async updateMeal(@Param('id') id: string, @Body() updateMealPayload: UpdateMealDto, @UploadedFile() file?: Express.Multer.File): Promise<{ message: string; updatedMeal: UpdateMealDto }> {
        const { updatedMeal } = await this.#_service.updateMeal(Number(id), updateMealPayload, file);
        return {
